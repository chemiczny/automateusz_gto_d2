...
def write_begin( cpp_file ):
    ...
    cpp_file.write( '#include "gto_d2_kit/d2_kinetic_matrix.hpp"\n')
...
def generate_suite_names( ):
    ...
    for shell1 in shells:
        for shell2 in shells:
            suite_names.append( 'kinetic_'+ shell1+ shell2)
            true_values[ 'kinetic_'+ shell1+ shell2 ] = []
...
def write_single_func( cpp_file, shell1, shell2, write_values = False ):
    ...
    cpp_file.write( 'void kinetic_' + shell1 + shell2 + '(){\n' )
    name = 'kinetic_' + shell1 + shell2
    ...
    cpp_file.write( '\tad2k['+shells2num[ shell1 ] +']['+shells2num[shell2] + ']( ae, be, cc, xAB, yAB, zAB, hxx, hxy, hxz, hyy, hyz, hzz, same_atom, matrix_size, Ai, Bi );\n')
...
def write_suite( cpp_file ):
    cpp_file.write( '\n')
    cpp_file.write( 'test_suite* test_kinetic(){ \n')
    cpp_file.write( '\ttest_suite * ts=BOOST_TEST_SUITE("Automateusz kinetic test");\n')
...
log_file = open( 'kinetic/LastTest.log', 'r' )
get_true_values( log_file )
log_file.close()
print true_values

cpp_file = open( 'kinetic/kinetic_test.cpp', 'w')
